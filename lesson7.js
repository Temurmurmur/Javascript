...
function move() {
    ...
    else if (direction == 'y-' && coord_y == 19){
        coord_y = -1;
    }
    else if (direction == 'x-' && coord_x == 0){
        coord_x = 20;
    }
    else if (direction == 'x+' && coord_x == 19){
        coord_x = -1;
    }
...
function haveLet(unit) {
    ...
    var unit_class = unit.getAttribute('class').split(' ');

    // Если препятствие
    if (unit_class.includes('let-unit')) {
        finishTheGame();
    }
    return check;
}
...
function createLet() {
    ...
    while (!letCreated) { //пока препятствие не создали
        ...
        if(LetCount > 5){
            var remLet = document.getElementsByClassName('let-unit')[0];
            remLet.classList.remove('let-unit');
            LetCount--;
        }
    }
}
